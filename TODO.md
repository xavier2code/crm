...
### 20. 报销管理
- [ ] 与产品确认是否保留该模块（路由 `/reimbursement` 与菜单已注册，但**后端无报销模块**）
- [ ] 如保留：补齐前后端（dashboard 已写"待审核报销 5"，需有真实数据源）
...
1. **补齐系统管理前端**（#8 用户/角色 + #9 数据权限 Controller + UI）— 让 admin 模块闭环
2. **客户/商机/合同前端**（#4 #5 #6）— 让核心业务流转可用
3. **修硬编码桩**（#6 ContractNodeService 1L/DEFAULT）
4. **跟进与日程**（#10 #11）— 日常操作入口
5. **通知中心 UI**（#16）— 站内信入口
6. **首次登录强制改密**（#17）— 安全合规
7. **中低优先级模块**（#12 返利 scheduler、#19 #20）— 按产品节奏逐步补齐
